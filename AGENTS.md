...
- **License**: Apache-2.0
- **MSRV**: Rolling "stable minus 2 releases" — lint against current stable, verify compilation against MSRV
...
- Use `just` commands instead of raw `cargo` in CI (e.g., `just test`, `just build-release`, `just coverage`)
- Use `jdx/mise-action` for toolchain setup — it installs all tools from `mise.toml`
- Pin all actions to full SHA with version comment (e.g., `@de0fac2e... # v6.0.2`)
- Pin `dtolnay/rust-toolchain` to master SHA with `toolchain:` input for version
- Never manually edit `release.yml` — it's autogenerated by cargo-dist
- Reference repos for workflow patterns: `../libmagic-rs` and `../stringy`
